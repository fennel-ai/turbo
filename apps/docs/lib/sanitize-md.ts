...
export function sanitize(page: string) {
	...
	return page.replace(REGEX, (_, tagName, properties, content) => {
		...
		while ((propMatch = propsRegex.exec(properties))) {
			...
			props.push({ key: key === 'style' ? 'type' : key, value: value.replace(/^(['"])(.*)\1$/, '$2') });
		}

		if (tagName === 'hint') {
			// If we have a hint tag, we take the capture group values and convert it to jsx syntax so MDX knows what to do with it.
			...
			return `<${tag} ${props.map(({ key, value }) => `${key}="${value}"`).join(' ')}>${content.trim()}</${tag}>`;
		} else {
